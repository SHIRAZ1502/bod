...
export const Incomes = () => {
    ...
    const onSubmit = (data) => {
      if(isOpen.action === "EDIT"){
        ...
        localStorage.setItem('incomes', JSON.stringify([...incomes, newIncomes]))
      }
      setIsOpen(false)
      setRating(0)
    }
    ...
    const deleteIncomes = (id) => {
        const newIncomes = incomes.filter((incomes) => incomes.id !== id)
        localStorage.setItem('incomes', JSON.stringify(newIncomes))
    ...
    const editIncomes = (id) => {
        const incomes = incomes.find((income) => income.id === id)
        setIsOpen({...isOpen, upsert: true, item:incomes})
    }
    ...
    <div>
        <div className='flex flex-col justify-center p-6 text-center'>
            ...
            {results.length?(
                ...
                <Transactions items={results} setIsOpen={setIsOpen} isOpen={isOpen}/>
            </div>
            ): (
                <div className='flex justify-center p-6 text-center'>
            ...
            {isOpen.upsert ? (
                ...
                setIsOpen = {setIsOpen} 
                isOpen = {isOpen} 
                title = {isOpen.action === "EDIT" ? 'Edit Incomes' : 'Add Incomes'} 
                onSubmit = {onSubmit}
                rating = {rating}
                setRating = {setRating}
                />
            ...
            {isOpen.delete ? (
                <DeleteModal
                    ...
                    title = 'Delete Income'
                    deleteItem = {() => deleteIncomes(isOpen.item.id)}
                />
